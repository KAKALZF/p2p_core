...
public class CalculatetUtil {
	...
	public static final BigDecimal NUMBER_MONTHS_OF_YEAR = new BigDecimal(
			"12.0000");
	public static final BigDecimal ACCOUNT_MANAGER_CHARGE_RATE = new BigDecimal(
			"0.0500");
	public static final BigDecimal INTEREST_MANAGER_CHARGE_RATE = new BigDecimal(
			"0.1000");
	...
	public static BigDecimal getMonthlyRate(BigDecimal yearRate) {
		...
		return yearRate.divide(ONE_HUNDRED).divide(NUMBER_MONTHS_OF_YEAR,
				Consts.CAL_SCALE, RoundingMode.HALF_UP);
	}
		...
		if (returnType == Consts.RETURN_TYPE_MONTH_INTEREST_PRINCIPAL) {// 按月分期
			...
			if (monthes2Return == 1) {
				...
				BigDecimal monthToReturnMoney = temp1.multiply(temp2).divide(
						temp3, Consts.CAL_SCALE, RoundingMode.HALF_UP);
				// 算出总还款
		...
		if (returnType == Consts.RETURN_TYPE_MONTH_INTEREST_PRINCIPAL) {// 按月分期
			...
			if (monthes2Return == 1) {
				...
				if (monthIndex < monthes2Return) {
					monthlyInterest = DecimalFormatUtil.amountFormat((temp1
							.subtract(monthToReturnMoney)).multiply(temp4)
